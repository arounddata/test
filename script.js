 ...
 * Версия 9-11 (исправлено добавление пустой строки)
 */
...
function krang(val, kubl, l, n, ib, ie) {
    ...
    if (!val || val.length === 0) return { val, kubl, k0, k1, k2, k3 };
    let swapped = true;
    while (swapped) {
        swapped = false;
...
function tmToCube(tmStr, n) {
    ...
    for (let i = 0; i < n; i++) {
        const attrNum = i + 1;
        let digit;
        if (determinants.includes(attrNum)) digit = 1;
        else if (functions.includes(attrNum)) digit = 2;
        else digit = 3;
        value |= (digit << (i * 2));
    }
    return value;
...
function kimpl1(kubList, n, kc1) {
    ...
    while (swout) {
        ...
        for (let i1 = ih1; i1 < ik1; i1++) {
            const x = kub[i1];
            let ih2 = (swi || (!swi && (i1 + 1 > k2 + k3))) ? i1 + 1 : k2 + k3;
            for (let i2 = ih2; i2 < ic; i2++) {
                const y = kub[i2];
                let j = 0, r = x & y, p = 7;
                for (let iBit = 0; iBit < g; iBit += 2) {
                    if (((r >> iBit) & 3) === 0) { j = iBit / 2; p++; }
                }
                if (p === 8 && j >= 0) {
                    r = r | (3 << (j * 2));
                    let swkub = 1;
        ...
        if (ir > 0) {
            ...
            let ib = 1, ie = ir;
            let res;
            if (swz) res = krang(vs, cz1, l, n, ib, ie);
            else res = krang(vs, cz2, l, n, ib, ie);
            vs = res.val;
            if (swz) cz1 = res.kubl;
            else cz2 = res.kubl;
            ir = res.k1;
            ...
            if (va.length !== ic) {
                ...
                for (let j = 0; j < kc1; j++) if (j < va.length) va[j] = 1;
            }
            for (let i = 0; i < ic; i++) {
                const x = kub[i];
                let swk2 = 1, swkub = 1;
                for (let i1 = 0; i1 < ir && swkub; i1++) {
                    ...
                    let p = 7, r = x & y;
                    for (let i2 = 0; i2 < g; i2 += 2) if (((r >> i2) & 3) === 0) p++;
                    if (p === 8) { if (i < va.length) va[i] = 2; swk2 = 0; break; }
                    else if (p === 7 && r === x) { if (i < va.length) va[i] = 0; swkub = 0; break; }
                }
            ...
            try {
                ...
                for (const elem of fdsiElement.children) {
                    if (elem.tagName.startsWith('fd')) {
                        ...
                        if (nums.length) maxAttr = Math.max(maxAttr, ...nums);
                    }
                }
                if (maxAttr === 0) reject(new Error("Не найдено ни одной ФЗ"));
                const n = maxAttr;
                const kubList = tmStrings.map(tmStr => tmToCube(tmStr, n));
                kubList.push(0);
...
function writeXmlResult(originalKubList, originalKc1, kubResult, ic, n) {
    ...
    const orderedOriginal = originalKubList.slice(0, originalKc1).filter(cube => originalSet.has(cube));
    const newCubes = kubResult.filter(cube => !originalSet.has(cube));
    const orderedResult = [...orderedOriginal, ...newCubes];
    const lines = ['<?xml version="1.0" encoding="UTF-8"?>', '<fds>', '<fdsi>'];
    for (let idx = 0; idx < originalKc1; idx++) {
        const cubeValue = originalKubList[idx];
        lines.push(`    <fd${idx + 1}>${cubeToTm(cubeValue, n)}</fd${idx + 1}>   <!-- ${cubeToStr(cubeValue, n)} -->`);
    }
...
function renderEditableTable() {
    ...
    let html = '<table class="fds-table"><thead><tr><th>№</th><th>ТМ-форма</th><th>Битовая форма</th></tr></thead><tbody>';
    for (let i = 0; i < appState.originalFds.length; i++) {
        const fd = appState.originalFds[i];
        const tmStr = fd.tm;
        let bStr = "—";
        if (tmStr && tmStr.trim() !== "") {
            try { bStr = cubeToStr(fd.cube, n); } catch(e) { bStr = "ошибка"; }
        }
        html += `<tr data-index="${i}">
            <td class="fd-number">${i + 1}</td>
            <td class="fd-tm" contenteditable="true">${escapeHtml(tmStr)}<\/td>
    ...
    editableCells.forEach(cell => {
        ...
        cell.addEventListener('focus', () => { oldValue = cell.innerText.trim(); });
...
function updateFdAt(index, newTm) {
    ...
    try {
        ...
        appState.closureResult = null;
        updateUI();
    } catch (e) { alert(`Ошибка в формате ФЗ: ${newTm}\n${e.message}`); renderEditableTable(); }
}

function addEmptyFd() {
    appState.originalFds.push({ tm: "", cube: 0 });
    appState.originalKc1 = appState.originalFds.length;
    appState.originalKubList = appState.originalFds.map(fd => fd.cube);
    appState.originalKubList.push(0);
    appState.resultSaved = false;
...
function loadFromFile(file) {
    parseXmlFile(file).then(({ kubList, n, kc1 }) => {
        ...
        for (let i = 0; i < kc1; i++) {
            ...
            appState.originalFds.push({ tm: cubeToTm(cube, n), cube });
        }
...
function getCurrentKubList() { return appState.originalFds.map(fd => fd.cube); }
...
async function saveAsFile() {
    ...
    } catch (err) { if (err.name !== 'AbortError') alert("Ошибка при сохранении: " + err.message); }
}
...
function updateUI() {
    ...
    if (appState.closureResult && appState.closureResult.length > 0) {
        ...
        for (let i = 0; i < orderedResult.length; i++) {
            ...
            html += `<tr><td class="fd-number">${i + 1}</td><td class="fd-tm">${cubeToTm(cube, n)}</td><td class="fd-comment">${cubeToStr(cube, n)}</td></tr>`;
        }
        html += '</tbody></table>';
...
document.getElementById('btnOpen').addEventListener('click', () => { fileInput.value = ''; fileInput.click(); });
document.getElementById('btnCalculate').addEventListener('click', calculate);
document.getElementById('btnSaveAs').addEventListener('click', saveAsFile);
document.getElementById('btnQuit').addEventListener('click', () => { if (confirm("Вы уверены, что хотите выйти?")) window.close(); });
...
document.addEventListener('keydown', (e) => {
    ...
    else if (e.ctrlKey && e.key === 'q') { e.preventDefault(); if (confirm("Вы уверены, что хотите выйти?")) window.close(); }
});
